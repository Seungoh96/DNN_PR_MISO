function [Pol_Tx_Opt, Pol_Rx_Opt, Pol_Tx_Opt_blk, Pol_Rx_Opt_blk] = Pol_JointPrePostCoding_test(H, Nt, Nr, N_iter)
%% Initialization 
...
%             theta_Tx(n_Tx) = -0.5*atan2(2*real(H_PD_Tx_temp(1,2)),(real(H_PD_Tx_temp(1,1))-real(H_PD_Tx_temp(2,2))));
            [S,A] = eig( real(H_PD_Tx_temp) );      
            A_PD_Vec = diag(A);
...
%             theta_Rx(n_Rx) = -0.5*atan2(2*real(H_PD_Rx_temp(1,2)),real((H_PD_Rx_temp(1,1))-real(H_PD_Rx_temp(2,2))));
            [S,A] = eig( real(H_PD_Rx_temp) );      
            A_PD_Vec = diag(A);
            [~,I_lamda_PD_max] = max(A_PD_Vec);
